...
export const ReferralItem: React.FC<ReferralItemProps> = ({ referral }) => {
  ...
  return (
    <div className='mt-[14px] flex h-10 w-full flex-row items-center justify-between'>
      <Text
        variant='b1'
        className='mr-2 flex h-full w-full items-center rounded-full border border-border-orange bg-bg-white px-4 text-text-black'
      >
        {referral.email}
      </Text>
      ...
      {isCompleted && !isClaimed && (
        <Button
          ...
          isLoading={isLoading}
        >
          <Text variant='b2' size='medium' className='text-text-white'>
            Claim Rewards
          </Text>
        </Button>
      )}
