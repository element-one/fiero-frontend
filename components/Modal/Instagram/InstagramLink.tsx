...
import { usePutLinkInstagram } from '@services/api/index'
...
export const InstagramLinkModal: React.FC<InstagramLinkModalProps> = ({
  onClose,
}) => {
  ...
  const { mutate, isLoading } = usePutLinkInstagram(undefined, {
    onSuccess: (response) => {
      setUser(response.user)
      toast.success('Instagram linked successfully to your account')
      closeModal()
    },
    onError: (err) => {
      let message
  ...
  return (
    <Transition
      appear
      show={isModalShown(ModalType.INSTAGRAM_LINK_MODAL)}
      as={Fragment}
    >
