...
import { usePostAddEventRewardCustom } from '@services/api'
import { useStore } from '@store/store'
import { ModalType, useModal } from 'contexts/modal'

interface EventCustomInfoModalProps {
  onClose?: () => void
}
...
export const EventCustomInfoModal: React.FC<EventCustomInfoModalProps> = ({
  onClose,
}) => {
  const eventReward = useStore((state) => state.eventReward)
  ...
  const { mutate: addEventCustom, isLoading } = usePostAddEventRewardCustom(
    eventReward?.id,
    {
      onSuccess: (response) => {
        setUserEventReward(response.userEventReward)
        hideModal()
  ...
  return (
    <Transition
      appear
      show={isModalShown(ModalType.EVENT_CUSTOM_INFO_MODAL)}
      as={Fragment}
    >
...
export default EventCustomInfoModal
